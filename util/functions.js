...
const coins = function (client, message) {
    ...
    console.log(chalk.blue(`[Coins Database]`) + ` Added ${addcoins} coins to ${message.author.tag} Server: ${message.guild.name} Channel: #${message.channel.name}`)
...
const logger = async function (client, type, err, msg, ch) {
    let e = new re()
        ...
        .setDescription(err)
        .setTimestamp()
        .addField(`Server`, `**Guild: **${type.name} (${type.id})`)
    if(ch){
